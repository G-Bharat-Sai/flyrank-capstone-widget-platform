...
public class WidgetController {
    ...
    function sha256Hex(str) {
                ...
                function renderWidget(script, widgetId, baseUrl, config) {
                    ...
                    if (config.requireDoubleOptIn) {
                        var consentLabel = document.createElement('label');
                        consentLabel.style.cssText = 'display:block;margin-top:8px;font-size:0.9em;';
                        consentCheckbox = document.createElement('input');
                        consentCheckbox.type = 'checkbox';
                        consentCheckbox.name = '_consent';
                        consentCheckbox.required = true;
                        consentLabel.appendChild(consentCheckbox);
                        consentLabel.appendChild(document.createTextNode(' I agree to be contacted and consent to my data being processed.'));
                    ...
                    form.addEventListener('submit', function(e) {
                        ...
                        if (consentCheckbox) {
                            payload.consent = consentCheckbox.checked;
                        }
                            ...
                            if (res.ok) {
                                return res.json().then(function(data) {
                                    if (data && data.confirmed === false) {
                                        message.textContent = 'Thanks! Please check your email to confirm your submission.';
                                    } else {
