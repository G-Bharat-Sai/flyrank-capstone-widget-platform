...
- Optional double opt-in / GDPR consent flow, opt-in per widget (`requireDoubleOptIn`): when enabled, a submission without a checked consent checkbox is rejected with a clean `400`; a consented submission is stored as unconfirmed until the visitor hits `GET /submissions/{id}/confirm` -- the link a real confirmation email would contain. No mail provider is wired up here, so the "email" is logged to the console instead of actually sent. The submission's own UUID `id` -- already handed to the visitor in the original response, and cryptographically unguessable -- doubles as their proof of ownership for two self-service endpoints: `GET /submissions/{id}/export` (their own stored data) and `DELETE /submissions/{id}` (erase it), without needing a separate auth system just for visitors.
...
2. Create a widget (`POST /widgets`) with a custom set of fields (name, email, whatever), a button label, optionally a webhook URL to get notified of new submissions, optionally proof-of-work bot protection (`requireProofOfWork`), optionally display targeting (`displayOptions.targetPages`, `delaySeconds`, `oncePerVisitor`), and optionally a double opt-in consent requirement (`requireDoubleOptIn`).
...
| GET | `/submissions/{id}/confirm` | none | Confirm a double-opt-in submission (the visitor's own UUID `id` is their proof of ownership) |
| GET | `/submissions/{id}/export` | none | A visitor exports their own submission data (GDPR) |
| DELETE | `/submissions/{id}` | none | A visitor deletes their own submission (GDPR) |
...
- Double opt-in / GDPR (`requireDoubleOptIn`) is real but intentionally minimal: the "confirmation email" is logged to the console rather than actually sent (no mail provider wired up), and the visitor's proof of ownership is simply knowing their own submission's UUID -- there's no separate visitor login or magic-link token system. That's a deliberate trade-off for a backend capstone: the consent gate, the unconfirmed-until-confirmed state, and the export/delete endpoints are all real and tested; a production system would still want an actual email send and probably a shorter-lived, single-use confirmation token instead of the permanent UUID.
